  ...
  const createMapView = useCallback(async () => {
    try {
      ...
      const stopGraphics = results.stopData.map((stop: any) => {
        return new Graphic({
          attributes: {
            name: stop.properties.matchstopdata && stop.properties.matchstopdata.name,
          },
          geometry: {
            type: 'point',
            longitude: stop.properties.matchstopdata && stop.properties.matchstopdata.lon,
            latitude: stop.properties.matchstopdata && stop.properties.matchstopdata.lat,
            spatialreference: {
              wkid: 4326,
            },
          },
